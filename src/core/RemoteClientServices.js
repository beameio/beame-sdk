 ...
 *
 * @param {Function} callback
 * @param {String|null} [edge_client_fqdn] remote edge client public fqdn (optional)
 * @param {Array.<string>|null} [ips] array of requested local ips  (optional)
 * @param {String|null} [authorization_atom_fqdn]  (optional)
 * @param {String|null} [authentication_atom_fqdn]  (optional)
 */
RemoteClientServices.prototype.createLocalEdgeClients = function (callback, edge_client_fqdn, ips, authorization_atom_fqdn, authentication_atom_fqdn) {
	...
	/**
	 *
	 * @param {Array.<String>} local_ips
	 */
	var requestHostnames = function (local_ips) {
	...
	if (ips) {
		requestHostnames(ips);
	}
	else {
		beameUtils.getLocalActiveInterfaces().then(
			/**
			...
			if (addresses.length) {
				requestHostnames(addresses);
			}
			else {
				callback({"message": "No local Ip's found"}, null);
			}
