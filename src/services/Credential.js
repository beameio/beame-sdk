...
class Credential {
	...
	initCryptoKeys() {
		...
		if (this.hasPrivateKey) {
			...
			this.privateKeyNodeRsa.importKey(this.privateKey + " ", "private");
		}
	}
...
//endregion
	...
	createEntityWithLocalCreds(parent_fqdn, name, email, validityPeriod, password) {
		return new Promise((resolve, reject) => {
				...
				api.setClientCerts(parentCred.privateKey, parentCred.P7B);
...
//endregion
	...
	revokeCert(signerAuthToken, signerFqdn, revokeFqdn) {
		return new Promise((resolve, reject) => {
				...
				if (!signerAuthToken) {
					...
					api.setClientCerts(cred.privateKey, cred.P7B);
				}
				else {
	...
	async _checkOcspStatus(cred, forceCheck = false) {
		...
		assert(cred.hasKey("X509"), `No certificate found for ${cred.fqdn}`);
...
//endregion
	...
	async checkValidity() {
		...
		assert(validity, `Certificate validity is not present for ${this.fqdn}`);
